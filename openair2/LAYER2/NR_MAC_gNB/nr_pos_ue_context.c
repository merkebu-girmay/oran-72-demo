...
void pos_act_store_ue_context(gNB_MAC_INST *mac, rnti_t rnti)
{
  elm_arr_t elm = find_if(&mac->pos_act_ue_arr, &rnti, eq_ue_rnti);
  if (elm.found) {
...
positioning_activation_info_t *pos_act_get_ue_context(gNB_MAC_INST *mac, rnti_t rnti)
{
  elm_arr_t elm = find_if(&mac->pos_act_ue_arr, &rnti, eq_ue_rnti);
  if (elm.found)
...
void pos_act_detach_ue_context(gNB_MAC_INST *mac, rnti_t rnti)
{
  elm_arr_t elm = find_if(&mac->pos_act_ue_arr, &rnti, eq_ue_rnti);
  if (elm.found) {
    seq_arr_erase(&mac->pos_act_ue_arr, elm.it);
  } else {
