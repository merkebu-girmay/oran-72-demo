                                               ...
                                               const rnti_t rnti)
{
  ...
  if (req->pos_report_characteristics) {
    f1ap_pos_measurement_periodicity_pr periodicity = req->measurement_periodicity;
    (void)periodicity;
  }
  ...
  const f1ap_pos_measurement_quantities_t *meas_q = &req->pos_measurement_quantities;
  ...
  f1ap_pos_measurement_result_list_t *meas_result_list = resp.pos_measurement_result_list;
      ...
      switch (q_item[j].pos_measurement_type) {
        case F1AP_POSMEASUREMENTTYPE_UL_RTOA:
          ...
          if (ta_offset_nsec[i] != (int16_t)0x8000) {
            ...
            if (num >= 0)
              k_value = (int32_t)((num + T_ns_inv / 2) / T_ns_inv) + k_offset;
            else
              k_value = (int32_t)((num - T_ns_inv / 2) / T_ns_inv) + k_offset;
  ...
  pos_act_detach_ue_context(mac, rnti);
